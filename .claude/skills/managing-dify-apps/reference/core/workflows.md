...
要件に基づいて選択してください。詳細は [../technical/templates.md](../technical/templates.md) を参照。

| 種類 | 推奨用途 | 複雑度 |
|------|--------|-------|
| **DeepResearch** | 深い調査、複数トピック研究 | ⭐⭐⭐⭐ 高度 |
| **Web検索と要約** | リアルタイム情報収集、要約 | ⭐⭐⭐ 中～高 |
| **投資分析** | 株価分析、市場調査 | ⭐⭐⭐⭐ 高度 |
| **知識リトリーバル** | ドキュメント検索、FAQ | ⭐⭐⭐ 中～高 |
| **質問分類器** | マルチカテゴリサポート | ⭐⭐⭐⭐ 高度 |

### ステップ3：適切なテンプレートを選択

Claudeが5つの公式テンプレートから選択するのをサポート：
- `DeepResearch.yml` → 深い調査、研究アシスタント
- `ウェブの検索と要約のワークフローパターン.yml` → Web検索と要約
- `投資分析レポート コパイロット.yml` → 金融・投資分析
- `知識リトリーバル + チャットボット.yml` → ナレッジベースQ&A
- `質問分類器 + 知識 + チャットボット.yml` → マルチサポート

詳細は [../technical/templates.md](../technical/templates.md) を参照。
